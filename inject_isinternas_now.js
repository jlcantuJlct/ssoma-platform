...
const isInternasBlock = `
            ...
            for (let i = 0; i < hallazgos.length; i++) {
                if (currentRow > 21) {
                    // Limiting to 7 static rows (15-21) to prevent ExcelJS corruption with merged cells
                    // Skip remaining to avoid corruption, or we can handle it differently.
                    // But for now, user just tests.
                    break;
                }
                const h = hallazgos[i];
...
// Inject before `// --- MANEJADOR 5: INSPECCIONES DIGITALES GENÉRICAS`
const injectionTarget = `        // --- MANEJADOR 5: INSPECCIONES DIGITALES GENÉRICAS`;
if (!code.includes('else if (isInternas)')) {
    ...
    console.log('Already has isInternas');
}
