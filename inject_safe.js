...
const target = `// --- MANEJADOR 5: INSPECCIONES DIGITALES GENÉRICAS`;

if (!code.includes('else if (isInternas) {')) {
    const lines = code.split('\n');
    const idx = lines.findIndex(l => l.includes(target));
    // Find the '}' immediately preceding the target
    let braceIdx = idx - 1;
    while(braceIdx > 0 && !lines[braceIdx].includes('}')) {
        braceIdx--;
    }
    
    // We will replace the '}' with the 'isInternasBlock' which starts with '} else if (isInternas) {'
    lines[braceIdx] = lines[braceIdx].replace('}', isInternasBlock);
