...
code = code.replace('export default function GeneradorInformes() {', compressFn + '\nexport default function GeneradorInformes() {');
...
const newAssign = `    const assignImage = async (tagName: string, originalFile: File) => {
        ...
        try {
            ...
            if (res.ok) {
                ...
                alert(\`No se pudo subir la foto a la nube. Si no tienes configurado Vercel Blob o hay un error: \${errData.error}\`);
                setTags(prev => prev.map(t => t.name === tagName ? { ...t, loading: false } : t));
            }
        } catch (e: any) {
