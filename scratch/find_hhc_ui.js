...
const res = lines.map((line, idx) => ({ line, idx })).filter(item => item.line.includes('Herramienta de Control') || item.line.includes('Horas Hombre') || item.line.includes('HHC') || item.line.includes('hhc'));

res.slice(250, 300).forEach(item => {
