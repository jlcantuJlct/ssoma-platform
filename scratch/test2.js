const { google } = require('googleapis');
const ROBOT_CREDENTIALS = {
    client_email: "obot-ssoma-nuevo@ssoma-app-485301.iam.gserviceaccount.com",
    private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
};

async function check() {
    const auth = new google.auth.GoogleAuth({
        credentials: { client_email: ROBOT_CREDENTIALS.client_email, private_key: ROBOT_CREDENTIALS.private_key.replace(/\\n/g, '\n') },
        scopes: ['https://www.googleapis.com/auth/drive.readonly']
    });
    const drive = google.drive({ version: 'v3', auth });
    try {
        const folderId = '13zLjByaLomXXVa3JJY9p8dCwUwewDIWi';
        const res = await drive.files.list({
            q: `'${folderId}' in parents and trashed=false`,
            fields: 'files(id, name, mimeType)',
            spaces: 'drive',
            supportsAllDrives: true,
            includeItemsFromAllDrives: true
        });
        console.log("Archivos dentro de Plantilla PAD SAN CLEMENTE Julio 2026:");
        console.log(JSON.stringify(res.data.files, null, 2));

        // Let's also check ROOT just in case Bridge uploaded them to ROOT:
        // By listing recently created files by user
        const res2 = await drive.files.list({
            q: `mimeType='image/jpeg' or mimeType='image/png'`,
            orderBy: 'createdTime desc',
            pageSize: 5,
            fields: 'files(id, name, parents, webViewLink)',
            supportsAllDrives: true,
            includeItemsFromAllDrives: true
        });
        console.log("\nÚltimos 5 archivos de imagen subidos al Drive:");
        console.log(JSON.stringify(res2.data.files, null, 2));
    } catch(e) { console.error("Error:", e.message) }
}
check();
