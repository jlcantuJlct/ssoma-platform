const { google } = require('googleapis');
const ROBOT_CREDENTIALS = {
    client_email: "obot-ssoma-nuevo@ssoma-app-485301.iam.gserviceaccount.com",
    private_key: "-----BEGIN PRIVATE KEY-----\\n-----END PRIVATE KEY-----\n"
};

async function moveFiles() {
    const auth = new google.auth.GoogleAuth({
        credentials: { client_email: ROBOT_CREDENTIALS.client_email, private_key: ROBOT_CREDENTIALS.private_key.replace(/\\n/g, '\n') },
        scopes: ['https://www.googleapis.com/auth/drive']
    });
    const drive = google.drive({ version: 'v3', auth });

    try {
        const rootId = '1j6wEqCN3zU9lsGthKeRCo_a6X4UH6NU5';
        const targetFolderId = '13zLjByaLomXXVa3JJY9p8dCwUwewDIWi'; // Plantilla PAD SAN CLEMENTE Julio 2026

        console.log("Buscando imágenes en ROOT...");
        let pageToken = null;
        let filesMoved = 0;

        do {
            const res = await drive.files.list({
                q: `'${rootId}' in parents and (mimeType='image/jpeg' or mimeType='image/png' or mimeType='image/jpg' or mimeType='application/octet-stream') and trashed=false`,
                fields: 'nextPageToken, files(id, name, parents)',
                spaces: 'drive',
                pageToken: pageToken,
                supportsAllDrives: true,
                includeItemsFromAllDrives: true,
                pageSize: 1000
            });

            const files = res.data.files;
            if (files && files.length > 0) {
                console.log(`Moviendo ${files.length} archivos...`);
                for (let i = 0; i < files.length; i += 10) {
                    const batch = files.slice(i, i + 10);
                    await Promise.all(batch.map(async (file) => {
                        try {
                            const previousParents = file.parents.join(',');
                            await drive.files.update({
                                fileId: file.id,
                                addParents: targetFolderId,
                                removeParents: previousParents,
                                fields: 'id, parents',
                                supportsAllDrives: true
                            });
                            filesMoved++;
                        } catch (err) {
                            console.error(`Error moviendo ${file.name}:`, err.message);
                        }
                    }));
                    console.log(`Movidos ${filesMoved}...`);
                }
            }

            pageToken = res.data.nextPageToken;
        } while (pageToken);

        console.log(`¡Listo! Se movieron ${filesMoved} archivos a la carpeta correcta.`);

    } catch(e) { 
        console.error("Error:", e.message);
    }
}
moveFiles();
