...
async function check() {
    const auth = new google.auth.GoogleAuth({
        credentials: {
            ...
            private_key: ROBOT_CREDENTIALS.private_key.replace(/\\n/g, '\n')
        },
    ...
    try {
        const res = await drive.files.get({
            fileId: '1j6wEqCN3zU9lsGthKeRCo_a6X4UH6NU5',
            fields: 'id, name, owners, shared, permissions, webViewLink'
        });
        console.log(JSON.stringify(res.data, null, 2));
    } catch(e) { console.error("Error root:", e.message) }
}
check();
