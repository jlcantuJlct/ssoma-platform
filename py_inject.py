...
lines = code.split('\\n')
target = '// --- MANEJADOR 5: INSPECCIONES DIGITALES GENÉRICAS'

if 'else if (isInternas) {' not in code:
    idx = -1
    ...
    if idx != -1:
        brace_idx = idx - 1
        while brace_idx > 0 and '}' not in lines[brace_idx]:
            brace_idx -= 1
            
        lines[brace_idx] = lines[brace_idx].replace('}', isInternasBlock)
