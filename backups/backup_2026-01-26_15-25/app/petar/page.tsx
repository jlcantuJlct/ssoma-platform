...
export default function PetarPage() {
    ...
    useEffect(() => {
        ...
        const loadRecords = async () => {
            // Fallback from localStorage for now
            const stored = localStorage.getItem('petar_records');
            if (stored) {
                try {
                    setRecords(JSON.parse(stored));
                } catch (e) {
                    console.error("Error parsing petar_records", e);
                }
            }
        };
        loadRecords();
    ...
    const handleSubmit = (e: React.FormEvent) => {
        ...
        setForm(prev => ({ ...prev, type: 'Caliente' })); // Reset basic fields
        setFile(null);
