                ...
                <nav className="flex-1 p-4 space-y-2 overflow-y-auto scrollbar-hide">
                    ...
                    <div className="space-y-1 mb-2">
                        ...
                        {process.env.NODE_ENV === 'development' && (
                            <button 
                                onClick={async () => {
                                    alert('Enviando actualizacion a Vercel. Por favor espere...');
                                    try {
                                        const res = await fetch('/api/deploy', { method: 'POST' });
                                        const data = await res.json();
                                        if (data.success) {
                                            alert(data.message || 'Exito: Los cambios han sido enviados a Vercel. Tardara unos minutos en reflejarse publicamente.');
                                        } else {
                            ...
                            >
                                <div className="flex items-center gap-3 px-3 py-2.5 rounded-xl transition-all duration-200 text-slate-400 hover:text-white hover:bg-slate-800/50 group">
                                    <div className="text-yellow-400 group-hover:text-yellow-300 group-hover:scale-110 transition-transform">
                                    ...
                                    <span className="font-medium text-xs">Actualizar Vercel</span>
                                </div>
                            </button>
                        )}
                    </div>
                    ...
                    <div className="relative group">
                        ...
                        <div className="relative border border-slate-800 bg-slate-900 rounded-lg p-3 flex items-center gap-3 transition-colors group-hover:border-emerald-500/30">
                            ...
                            <div className="flex flex-col">
                                ...
                                <span className="text-[10px] text-green-500 font-bold font-mono mt-0.5">v2026.5.07 - ASSISTANT v1.2 & EPP UI</span>
                            </div>
                        </div>
                    </div>
                </div>
