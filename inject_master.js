...
// 1. Read fresh restored code
let code = fs.readFileSync(path, 'utf8');

// 2. Define the perfect isInternas block
const cleanInternas = `else if (isInternas) {
            const getAns = (label) => {
...
// 3. Re-inject it right before MANEJADOR 4 (which is where isInternas goes)
const injectionTarget = `        // --- MANEJADOR 4: MAQUINARIA Y EQUIPO PESADO ---`;
if (code.includes(injectionTarget)) {
    code = code.replace(injectionTarget, cleanInternas + '\\n\\n' + injectionTarget);
} else {
    console.error("COULD NOT FIND MANEJADOR 4");
}

// 4. Inject matchedRows globally at the top
code = code.replace('export async function POST(req: Request) {', 'export async function POST(req: Request) {\\n    let matchedRows: any;');

// 5. Also need to inject isCampamento and isElectricas because git restore wiped them!
// Wait! `git restore` wiped EVERYTHING that was not committed!
// I need to apply `route_patch_utf8.diff` first!
